 ...
 *
 */
export function parseCssValue(d, index, width, height) {
	if (!d) return 0;
	if (typeof d === "number") {
		return d;
	}
	if (d.includes("%")) {
		return (+d.replace("%", "") / 100) * (index ? height : width);
	}
	return +d.replace("px", "");
}
 ...
 *
 */
export function getElPosition(element) {
	const annotationBbox = element.getBoundingClientRect();
	const parentBbox = element.parentNode.getBoundingClientRect();
...
export function swoopyArrow() {
	...
	function render(data) {
		data = data.map((d, index) => {
			return [xValue.call(data, d, index), yValue.call(data, d, index)];
		});
      ...
      */
		...
		if (vertical) {
			var path = "M" + data[0][0] + "," + data[0][1] + "V" + data[1][1];
		} else {
			var path =
				"M" +
				data[0][0] +
	...
	render.clockwise = function renderClockwise(_) {
		if (arguments.length === 0) return clockwise;
		clockwise = !!_;
		return render;
	};

	render.x = function renderX(_) {
		if (arguments.length === 0) return xValue;
		xValue = _;
		return render;
	};

	render.y = function renderY(_) {
		if (arguments.length === 0) return yValue;
		yValue = _;
		return render;
	};

	render.vertical = function renderVertical(_) {
		if (arguments.length === 0) return vertical;
		vertical = _;
		return render;
	};
