...
function ramp(color, n = 256) {
	...
	for (let index = 0; index < n; ++index) {
		context.fillStyle = color(index / (n - 1));
...
export function Legend(
		...
		if (!x.ticks) {
			if (tickValues === undefined) {
				...
				tickValues = d3
					.range(n)
					.map((index) =>
						d3.quantile(color.domain(), index / (n - 1))
					);
			}
	...
	// Threshold
		...
		svg.append("g")
			...
			.data(color.range())
			.join("rect")
			.attr("x", (d, index) => x(index - 1))
			.attr("y", marginTop)
			.attr("width", (d, index) => x(index) - x(index - 1))
			.attr("height", height - marginTop - marginBottom)
			.attr("fill", (d) => d);

		tickValues = d3.range(thresholds.length);
		tickFormat = (index) => thresholdFormat(thresholds[index], index);
	}
